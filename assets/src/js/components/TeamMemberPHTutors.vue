<template>
  <v-container fluid style="height:100%">
    <v-card style="height:100%">
      <v-card flat>
        <v-card-title>Ph tutors</v-card-title>

        <v-data-table class="mr-3" :headers="headers">
          <template v-slot:body>
            <tbody v-for = "(member, index) in Tutors">
              <tr>
                <td>{{index+1}}</td>
    ...
    <v-container>
      ...
      <v-row class="pb-0">
        <v-col cols="12" sm="6" md="3" class="pb-0">
          <v-text-field    class="pb-0" label="Mega Name" v-model="editTutor.mega_name" placeholder="Mega Name"></v-text-field>
        </v-col>
        <v-col cols="12" sm="6" md="3" class="pb-0">
          <v-text-field    class="pb-0" label="Position" v-model="editTutor.position" placeholder="Position"></v-text-field>
        </v-col>
        <v-col cols="12" sm="6" md="3" class="pb-0">
          <v-text-field   class="pb-0" label="First Name" v-model="editTutor.first_name" placeholder="First Name"></v-text-field>
        </v-col>
        <v-col cols="12" sm="6" md="3" class="pb-0">
          <v-text-field    class="pb-0" label="Last Name" v-model="editTutor.last_name" placeholder="Last Name"></v-text-field>
        </v-col>
      </v-row>

      <v-row class="py-0">
        <v-col cols="12" sm="6" md="3" class="personal py-0">
          <v-text-field   label="Sex" v-model="editTutor.sex" placeholder="Sex"></v-text-field>
        </v-col>
        <v-col cols="12" sm="6" md="3" class="personal py-0">
          <v-text-field    label="Date of Birth" v-model="editTutor.date_of_birth" placeholder="Date of Birth"></v-text-field>
        </v-col>
        <v-col cols="12" sm="6" md="3" class="personal py-0">
          <v-text-field    label="Mobile Number" v-model="editTutor.mobile_number" placeholder="Mobile Number"></v-text-field>
        </v-col>
        <v-col cols="12" sm="6" md="3" class="personal py-0">
          <v-text-field    label="Email"  v-model="editTutor.email" placeholder="Email"></v-text-field>
        </v-col>
      </v-row>

      <v-row class="py-0">
        <v-col cols="12" lg="12" class="py-0">
          <v-text-field  label="Residential Address" v-model="editTutor.address" placeholder="Residential Address"></v-text-field>
        </v-col>
      </v-row>

      <v-row class="py-0 mb-5">
        <v-col cols="12" md="6" class="py-0">
          <v-text-field    label="Major" v-model="editTutor.major" placeholder="Major"></v-text-field>
        </v-col>
        <v-col cols="12" md="6" class="py-0">
          <v-text-field    label="School" v-model="editTutor.school" placeholder="School"></v-text-field>
        </v-col>
      </v-row>
      ...
      <v-row class="pb-0">
        ...
        <template v-for="(work, index) in editTutor['workexp']">
          <v-col cols="12" sm="6" md="3" class="pb-0">
            <v-text-field    label="Position" v-model="editTutor['workexp'][index]['Position']" placeholder="Position"></v-text-field>
          </v-col>
          <v-col cols="12" sm="6" md="3" class="pb-0">
            <v-text-field    label="Company Name" v-model="editTutor['workexp'][index]['Company']"placeholder="Company Name"></v-text-field>
          </v-col>
          <v-col cols="12" sm="6" md="3" class="pb-0">
            <v-text-field   label="From" v-model="editTutor['workexp'][index]['Start date']" placeholder="From"></v-text-field>
          </v-col>
          <v-col cols="12" sm="6" md="3" class="pb-0">
            <v-text-field   label="To"  v-model="editTutor['workexp'][index]['End date']"placeholder="To"></v-text-field>
          </v-col>
      ...
      <v-row>
        <v-col cols="12" md="12" class="py-0">
          <v-textarea   name="input-7-4" label="Introduction" v-model="editTutor.introduction" placeholder="Introduce yourself as a teacher here"></v-textarea>
        </v-col>
      </v-row>
      <v-row>
        <v-col cols="12" sm="12" class="pb-0">
          <v-text-field    label="Specialty" v-model="editTutor.specialty" placeholder="Specialty"></v-text-field>
        </v-col>
      </v-row>
           ...
           >
             <template v-slot:selection="{ index, text }">
               ...
               <v-row>
                 <v-spacer></v-spacer>
                 <div class="my-2">
                      <v-btn color="#E91E63" class="white--text" @click = "close()">Close</v-btn>
                   <v-btn color="#E91E63" @click = "updateTutor()"class="white--text">Save and Proceed</v-btn>
                 </div>
...
mounted(){
  ...
  axios.get('Tutor/get_ph_tutors').then(res => {
...
updateTutor(){
  ...
  const editTutorData = this.editTutor
...
this.editTutor.files = this.files
  }
  let stringdata = []

  editTutorData.workexp.forEach((item, i) => {
      stringdata.push('Position: ' + item['Position'] + " " +'Company: ' + item['Company'] + " " + 'Start date: ' + item['Start date'] + " " + 'End date: ' + item['End date'])
  });

  editTutorData.workexp = stringdata
  this.$store.dispatch('updateTutor', {Tutor: editTutorData})
}
},
